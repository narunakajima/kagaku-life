...
# 2026-09-14: PROMPT_TEMPLATEの冒頭を「AI・ロボティクス分野」→「科学技術分野」に
# 一般化したが、採点基準・重み・判定ロジックは一切変更していない（AI・ロボティクス
# 論文への判定結果は従来と同じになるはずの、対象分野を広げるだけの変更）ため、
# STAGE4_VERSIONは意図的に据え置いた。在庫480件超の全件再採点という高コストな
# 処理を、実質的に判定結果が変わらない変更のために強制しないための判断。
...
描くチャンネルです（2026-09-14まではAI・ロボティクス分野に限定していたが、分野の限定は
撤廃した。AI・ロボティクスの研究はもちろん、遺伝子治療・創薬・材料科学・気候科学等、
分野を問わず対象になる）。以下の論文が次エピソード候補としてどれくらい「面白いか」を
判定してください。
