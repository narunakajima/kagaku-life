...
- **`closing`の末尾2文は固定（2026-08-25追加）:** 情緒的な着地の直後に、
  (1)「詳しい研究内容が知りたい方は概要欄をご確認ください」の趣旨、
  (2)チャンネル登録を促す一文、を必ず加える（文言はエピソードのトーンに
  合わせて言い換えてよいが、内容は固定）。**(2)は単純な「登録お願いします」
  ではなく、次回予告的に「これからも○○を一緒に見つけていきませんか」等、
  登録する理由（今後もこの番組の研究紹介に立ち会える）を持たせる文言にする
  （2026-08-25改訂、kl004の実例参照）。**
