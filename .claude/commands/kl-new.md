...
5. **シーン間の接続**（2026-08-31追加）: 各シーンをJSON順に通し読みし、
   指示語（「この研究」「その技術」等）が、それより前のシーンで一度も
   話題に上っていないものを既出であるかのように参照していないか確認する。
   典型例はcitationシーンの書き出し（例:「この研究を進めているのは」）が、
   直前のcontextシーンではまだ研究そのものに触れていないのに唐突に「この
   研究」と始めてしまうケース（kl009で実際に発生、ユーザー指摘で発覚）。
   文法的には誤りがなく単体で読むと不自然に見えないため、観点1〜4の
   チェックだけでは見落とされやすい。シーンをまたぐ話題の受け渡しが
   自然かを意識して通し読みすること。
