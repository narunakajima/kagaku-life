...
## STEP 2.5 — 台本の品質・配慮チェック（Opusサブエージェントに委任、2026-08-22追加）
...
では気づけないことがkl001で実際に確認されている**（Sonnet生成→人間との複数
往復のやり取りでようやく修正できた）。SCのSTEP3A（史実知識チェック、Opus使用）
と同じ理由でここだけ強いモデルを使う。
...
1. **特定の属性への配慮**（学歴・職業・年齢・家族構成等）: 比較対象となる属性を
   ...
   劣ったものとして描いていないか）。研究内容の紹介として正確な範囲を超えて、
   特定の読者層を傷つける書き方になっていないか。
