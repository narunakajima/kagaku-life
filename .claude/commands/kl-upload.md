...
**デフォルト動作: 直近の土曜 21:00 JST に自動予約。**
次の土曜21:00までにアップロードされたエピソードは全てその土曜21:00にまとめて公開される
...
**日時を手動指定（JST）:**
```bash
python3 $HOME/kagaku-life/kl_sns_up.py --episode kl{NNN} --publish-at "2026-08-29 21:00"
```
...
1. 今日が土曜かつ21:00 JSTがまだ未来 → 今日を候補に
2. それ以外 → 次の土曜を候補に
3. **次の土曜21:00までにアップロードされたエピソードは全てその土曜21:00にまとめて
   公開する**（1週1本ではなく、複数エピソードが同じ土曜のスロットを共有してよい）

## STEP 3 — 完了報告

```
✓ アップロード完了（予約公開: 2026-08-29 21:00 JST（自動））
