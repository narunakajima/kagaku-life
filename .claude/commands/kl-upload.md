...
**デフォルト動作: 毎週土曜の 19:00 JST に1本自動予約。**
複数エピソードが積まれている場合は次の空き土曜に自動でずらす。
...
スロット割り当てロジック:
1. `episodes/kl*.json` の `scheduled_at` を読んで使用済み日付を収集
2. 今日が土曜かつ19:00 JSTがまだ未来 → 今日を候補に
3. それ以外 → 次の土曜以降の空き日を自動で割り当て
